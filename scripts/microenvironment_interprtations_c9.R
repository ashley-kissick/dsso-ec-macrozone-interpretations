...
num_clusters <- length(feature_clusters)
...
names(summary_list) <- paste0("cluster_", 1:num_clusters)
...
#If we remove the little areas, do we still have representation
#across zones?

length(unique(flagged_df$label))
length(unique(high_percent$label))


#This drops from 39 to 23 labels across the fields, losing 
#16 'unique' environments.  

#Let's take a look to see:
all_unique <- unique(flagged_df$label)
unique_flagged <- unique(high_percent$label)
                    
dropped_labels <- which(all_unique %in% unique_flagged == FALSE)
dropped_labels <- all_unique[dropped_labels] 
dropped_labels

#Now, get the cluster assignments to see if any belong in the 
#same cluster:

dropped_sum <- summaries[which(summaries$label %in% dropped_labels == TRUE),]
dropped_sum[,c(1:7, 9, 11)]
#Looks like cluster 1 and cluster 4 would be lost.  
#Cluster 5, 6, 8, and 9 have general representation by similarity.

#Let's maybe create a rule-based system so that if a small area is detected, it is combined 
#with its counterpart.  I don't think they are combined 
...
#Cluster 9 logic:  
c_2 <- check_repeats[which(check_repeats$Group_ID == "cluster_2"), 1:3]
c_2

c_6 <- check_repeats[which(check_repeats$Group_ID == "cluster_6"), 1:3]
c_6

c_8 <- check_repeats[which(check_repeats$Group_ID == "cluster_8"), 1:3]
c_8 

c_9 <- check_repeats[which(check_repeats$Group_ID == "cluster_9"), 1:3]
c_9

#cluster 2 (label 072 and 071)
#cluster 6 (label 142, 141), (141, 142, 136) (same as cluster 9 with cluster 14 logic)
#cluster 8 (label 096, 090, 095), (090, 096) (same as cluster 12 with cluster 14 logic)
#cluster 9 
#(101, 148)
#(148,146, 100)
#(101, 146, 148)
